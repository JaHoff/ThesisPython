...
styles = [None, None,None, None, '-d', '-d', None, None, None, None, '-o','-o','-o','--', '-d', '--']
...
PF.Matrix2DPlot(t,MDRelPos[:,0,1:moonborder], title='',
...
for i in range(1,moonborder):
        a,b, texts = PF.Simple2DScatterAdjustedText(scatX[i],scatY[i],title='',
                               xlabel = 'Final change in absolute position (#1) [m]', 
                               ylabel = 'Final change in baseline [m]', xmod = 1.,
                               ymod = 1., xlim = [5E-2,1E10], ylim = [1E-3,5E6] , fig = fig,
                               ax = ax, addAnnotation = True, annotation = legend[i] ,texts = texts)
#Changes in relative baseline against absolute position from perturbating forces
pp.axhline(0.10,ls = ':', color='r') 
pp.axvline(10, ls = ':', color='r')
pp.text(1e6, 0.11, "10cm baseline uncertainty limit" , color = 'r'  )
pp.text(1.1e1, 1e3, "10m position uncertainty limit", color = 'r'  )
...
for i in range(1,moonborder):
        a,b, texts = PF.Simple2DScatterAdjustedText(scatX[i],scatY[i],title='',
                               xlabel = 'Maximum change in absolute position (#1) [m]', 
                               ylabel = 'Maximum change in baseline [m]', xmod = 1.,
                               ymod = 1., xlim = [5E-2,1E10], ylim = [1E-3,5E6] , fig = fig,
                               ax = ax, addAnnotation = True, annotation = legend[i] ,texts = texts)
#Maximum change in relative baseline against absolute position from perturbating forces
pp.axhline(0.10, ls = ':', color='r',zorder = 0) 
pp.axvline(10, ls= ':', color='r' , zorder = 0)
pp.text(1e6, 0.11, "10cm baseline uncertainty limit" , color = 'r'  )
pp.text(1.1e1, 1e3, "10m position uncertainty limit", color ='r'  )
adjust_text(texts,arrowprops=dict(arrowstyle='-', color='black'))
